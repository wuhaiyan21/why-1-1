...
with col2:
    ...
    if chart_mode != "仅显示总额":
        ...
        selected_categories = st.multiselect(
            "选择分类进行对比（请至少选择 2 个分类）",
            CATEGORIES,
            default=default_cats,
        )
        if len(selected_categories) < 2:
            st.warning("⚠️ 请至少选择 2 个分类进行对比。")
    else:
        selected_categories = []

    trend_data_long = []
    show_total = chart_mode == "仅显示总额" or chart_mode == "按分类对比（叠加总额）"
    show_categories = chart_mode != "仅显示总额" and len(selected_categories) >= 2
    ...
    if trend_data_long:
        ...
        if len(selected_categories) >= 2 and chart_mode != "仅显示总额":
            color_map = {}
